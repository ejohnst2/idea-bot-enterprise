...
function getClientByTeamId(teamId) {
  if(!client[teamId] && botAuthorizations[teamId]) {
    client[teamId] = new SlackClient(botAuthorizations[teamId])
  }
  if (clients[teamId]) {
...
slackEvents.on('message', (message, body) => {
  ...
  if (!slack) {
    return console.log('No Authorization found for this yteam')
  }
  slack.chat.postMessage({ channel: message.channel, text: `Hello <@${message.user}>! :tada:` }).catch(console.error)
})

slackEvents.on('react_added', (event, body) => {
  const slack = getClientByTeamId(body.team_id)
  ...
  slack.chat.postMessage(event.item.channel, `${event.reaction}:`).catch(console.error)
})
